...
function Posts() {
  ...
  const handleDeletePost = async (postId) => {
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.errors?.join(', ') || errorData.error || 'Failed to delete post');
      }
      ...
      {posts.map(post => (
        <div className="post" key={post.id}>
          ...
          <div className="post-text">
            ...
            <button className="delete-post" onClick={() => handleDeletePost(post.id)}><AiOutlineDelete /></button>
            <button className="delete-post" onClick={() => handleDeletePost(post.id)}><CiEdit/></button>
          </div>
